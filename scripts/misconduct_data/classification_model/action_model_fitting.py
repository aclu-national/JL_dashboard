...
# Loading data
misconduct = r.misconduct_action

# Defining dataframes
X = misconduct[['action', 'agency']]
y = misconduct['action_classified']

# Splitting the data using a 50% train-test split and a random state of 1
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.5, random_state=1)

# Create a transformer to convert actions to a matrix of TF-IDF features
...
X_test_uncommon = X_test[-X_test['action'].isin(common_actions)]
y_test_uncommon = y_test.loc[X_test_uncommon.index]

X_test_common = X_test[X_test['action'].isin(common_actions)]
y_test_common = y_test.loc[X_test_common.index]



# Get predictions for the common actionsX_test_uncommon
...
if best_model is not None:
    joblib.dump(best_model, 'scripts/misconduct_data/classification_models/finished_models/action_best_model.joblib')
else:
