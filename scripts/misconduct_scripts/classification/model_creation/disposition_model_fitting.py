...
# Loading data
misconduct = r.misconduct_disposition

# Defining dataframes
X = misconduct[['disposition', 'agency']]
y = misconduct['disposition_classified']

# Splitting the data using a 50% train-test split and a random state of 1
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.5, random_state=1)

# Create a transformer to convert dispositions to a matrix of TF-IDF features
...
preprocessor = ColumnTransformer(
    transformers=[
        ('text', text_transformer, 'disposition'),
        ('categorical', categorical_transformer, ['agency'])
    ])
...
common_dispostions = list(set(X_train["disposition"]) & set(X_test["disposition"]))

# Filter test set to include only common dispositions
X_test_uncommon = X_test[-X_test['disposition'].isin(common_dispostions)]
y_test_uncommon = y_test.loc[X_test_uncommon.index]

X_test_common = X_test[X_test['disposition'].isin(common_dispostions)]
y_test_common = y_test.loc[X_test_common.index]




# Get predictions for the common dispositionsX_test_uncommon
...
if best_model is not None:
    joblib.dump(best_model, 'scripts/misconduct_data/classification_models/finished_models/disposition_best_model.joblib')
else:
